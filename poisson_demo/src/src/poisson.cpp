...
class PoissonControllerNode : public rclcpp::Node{
    ...
    private:
        ...
        void mpc_callback(void){
            ...
            if(predictive_sf_flag && h_flag && mpc_mutex.try_lock()){
                ...
                for(int i=0; i<MAX_SQP_ITERS; i++){
                    mpc3d_controller.line_search(hgrid1, dhdt_grid, xc, grid_age, wn, buff);
                    mpc3d_controller.update_cost(vn);
                    mpc3d_controller.update_constraints(hgrid1, dhdt_grid, x, xc, grid_age, wn, issf, buff);
                    mpc3d_controller.solve();
        ...
        void find_and_lock_boundary(float *grid, float *bound){
            ...
            const int imin = buff;
            const int jmin = buff;
            const int imax = IMAX-1-buff;
            const int jmax = JMAX-1-buff;
            for(int i = 0; i < IMAX; i++){
                for(int j = 0; j < JMAX; j++){
        ...
        void compute_fast_forcing_function(float *force, const float *bound){
            ...
            for(int i = buff+1; i < IMAX-buff-1; i++){
                for(int j = buff+1; j < JMAX-buff-1; j++){
        ...
        void display_poisson_safety_function(void){
            ...
            for (int i = buff; i < IMAX-buff; i++){
                for (int j = buff; j < JMAX-buff; j++){
                    poisson_img.at<float>(i-buff,j-buff) = grid_temp[i*JMAX+j];
                }
            }
            ...
            cv::Point curr_pt = cv::Point(upscale*(x_to_j(x[0],xc[0])-buff),upscale*(y_to_i(x[1],xc[1])-buff));
            ...
            for(int n = 1; n < TMAX; n++){
                ...
                cv::Point traj_pt = cv::Point(upscale*(j_traj-buff), upscale*(i_traj-buff));
            ...
            for(int q = 0; q<QMAX; q++){
                for(int i = 0; i<IMAX; i++){
                    for(int j = 0; j<JMAX; j++){
                        ...
                        float dhdt_ij = 0.0f;
                        if(in_grid){
                            const float h0 = trilinear_interpolation(hgrid0, i0, j0, (float)q);
                            const float h1 = trilinear_interpolation(hgrid1, (float)i, (float)j, (float)q);
        ...
        void safety_filter(const std::vector<float> vd){
            ...
            const float ir = y_to_i(x[1], xc[1]);
            const float jr = x_to_j(x[0], xc[0]);

            // Saturated Because of Finite Grid Size
            const float qc = yaw_to_q(x[2], xc[2]);
            const float ic = std::clamp(ir, (float)buff, (float)(IMAX-buff-1)); // Saturated Because of Finite Grid Size
            const float jc = std::clamp(jr, (float)buff, (float)(JMAX-buff-1)); // Numerical Derivatives Shrink Effective Grid Size            
            ...
            float h_pred = h + dhdt * grid_age;
        
            // Compute Current Gradient
            const float i_eps = 5.0f;
            const float j_eps = 5.0f;
            const float q_eps = 1.0f;
            const float ip = std::clamp(ic + i_eps, (float)buff, (float)(IMAX-buff-1));
            const float im = std::clamp(ic - i_eps, (float)buff, (float)(IMAX-buff-1));
            const float jp = std::clamp(jc + j_eps, (float)buff, (float)(JMAX-buff-1));
            const float jm = std::clamp(jc - j_eps, (float)buff, (float)(JMAX-buff-1));
            ...
            float dhdx_pred = (hxp-hxm) / (2.0f*j_eps*DS);
            float dhdy_pred = (hyp-hym) / (2.0f*i_eps*DS);
            float dhdq_pred = (hqp-hqm) / (2.0f*q_eps*DQ);
