    ...
    public: 
        ...
        int update_constraints(const float *x0, const float *hgrid){
            ...
            for (int k=0; k<=N_HORIZON; k++){
                ...
                const float h = bilinear_interpolation(hgridk, ic, jc);
                const float dhdx = (bilinear_interpolation(hgridk, ic, jp) - bilinear_interpolation(hgridk, ic, jm)) / (2.0f * x_eps);
                const float dhdy = (bilinear_interpolation(hgridk, ip, jc) - bilinear_interpolation(hgridk, im, jc)) / (2.0f * y_eps);
        ...
        void set_input(float *vx, float *vy, float age){

            const float vx0 = sol(nX+0);
            const float vy0 = sol(nX+1);

            const float vx1 = sol(nX+2);
            const float vy1 = sol(nX+3);
