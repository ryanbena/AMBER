...
namespace Kernel{
    ...
    __constant__ int nElem = IMAX * JMAX * TMAX;
    ...
    int Poisson(float *h, const float *f, const float *b, const float relTol, const float w_SOR){
        ...
        // Memory sizes
        size_t grid_size = IMAX * JMAX * TMAX * sizeof(float);
        ...
        const int blocksPerGrid = (IMAX * JMAX * TMAX + threadsPerBlock - 1) / threadsPerBlock;
