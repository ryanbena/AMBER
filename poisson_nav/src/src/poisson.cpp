...
class PoissonControllerNode : public rclcpp::Node{
    ...
    public:
        ...
        PoissonControllerNode() : Node("poisson_control"), sport_req(this){
            ...
            pose_suber_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("aligned_pose", 1, std::bind(&PoissonControllerNode::state_update_callback, this, std::placeholders::_1), options2);
    ...
    private:
        ...
        void teleop_callback(geometry_msgs::msg::Twist::UniquePtr msg){
            ...
            // Integrate Goal Point & Yaw Wrap
            // xd[0] = x[0] + vt[0] * DT * (float)(TMAX-1);
            // xd[1] = x[1] + vt[1] * DT * (float)(TMAX-1);
            // xd[2] = x[2] + vt[2] * DT * (float)(TMAX-1);
            // xd[2] = 0.0f;
            // xd[2] = std::fmod(xd[2]-x[2]+M_PI, 2.0f*M_PI) + x[2] - M_PI;

            // If Checkpoint Reached, Update the Counter
            const float ex = xd[0] - x[0];
            const float ey = xd[1] - x[1];
            const float e = std::sqrt(ex*ex+ey*ey);
            if(start_flag && e<0.4f){
                if(chkpt_counter<3) chkpt_counter++;
                else chkpt_counter=0;
            }

            float checkpts[4][3] = {{0.0f, 0.0f, 0.0f},
                                    {2.5f, 0.0f, 0.0f},
                                    {0.0f, 1.7f, 3.14f},
                                    {-3.5f, 1.7f, 3.14f}};
            
            xd[0] = checkpts[chkpt_counter][0];
            xd[1] = checkpts[chkpt_counter][1];
            xd[2] = checkpts[chkpt_counter][2];
            xd[2] = std::fmod(xd[2]-x[2]+M_PI, 2.0f*M_PI) + x[2] - M_PI;

            for(int k=0; k<TMAX; k++){
                // xd_traj[3*k+0] = x[0] + vt[0] * k * DT;
                // xd_traj[3*k+1] = x[1] + vt[1] * k * DT;
                // xd_traj[3*k+2] = x[2] + vt[2] * k * DT;
                xd_traj[3*k+0] = xd[0];
                xd_traj[3*k+1] = xd[1];
                xd_traj[3*k+2] = xd[2];
            } 
        ...
        void state_update_callback(const geometry_msgs::msg::PoseStamped::SharedPtr data){
            ...
            // Log State
            Eigen::Vector3f r;
            r << data->pose.position.x, data->pose.position.y, data->pose.position.z;
            Eigen::Quaternionf q;
            ...
            //std::cout << "ROTM: <" << R(0,0) << "," << R(1,1) << "," << R(2,2) << ">" << std::endl;
