...
class PoissonControllerNode : public rclcpp::Node{
    ...
    public:
        ...
        PoissonControllerNode() : Node("poisson_control"){
            ...
            occ_big = (float *)malloc(360*360*sizeof(float));

            for(int n = 0; n < 360*360; n++){
                occ_big[n] = 1.0f;
            }
    ...
    private:
        ...
        void optical_flow_callback(std_msgs::msg::Float32MultiArray::UniquePtr msg){
            ...
            for(int n = 0; n < IMAX*JMAX; n++){
                occ_vi[n] *= 1.0f - kf;
                occ_vj[n] *= 1.0f - kf;
                occ_vi[n] += kf * msg->data[0*IMAX*JMAX+n] / dt_flow;
                occ_vj[n] += kf * msg->data[1*IMAX*JMAX+n] / dt_flow;
            }
        ...
        void optitrack_state_callback(geometry_msgs::msg::PoseStamped::SharedPtr data){
            ...
            if(h_flag) safety_filter(rx, ry, yaw, grid_age, true);
        ...
        float *f0, *hgrid, *vxgrid, *vygrid, *h0grid, *occ_big;
