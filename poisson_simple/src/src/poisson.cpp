...
class PoissonControllerNode : public rclcpp::Node{
    ...
    private:
        ...
        bool solve_poisson_safety_function(void){
            ...
            if(!MPC3D_FLAG) fill_elliptical_robot_kernel(robot_kernel, yaw, robot_kernel_dim, 2.0f);
        ...
        void display_poisson_safety_function(const float *grid){
        ...
        void safety_filter(const bool filter_flag){
            ...
            // Fractional Indices Corresponding to Current Position
            const float ic = y_to_i(ry, ryc);
            const float jc = x_to_j(rx, rxc);
            const float ic0 = ic + dry / DS;
            const float jc0 = jc + drx / DS;

            // Fractional Indices Corresponding to Current Orientation
            const float qr = yaw_to_q(yaw,yawc);
            const float qc = q_wrap(qr);

            // Get Safety Function Rate
            const float h0 = trilinear_interpolation(hgrid0, ic0, jc0, qc);
            h = trilinear_interpolation(hgrid1, ic, jc, qc);
            dhdt = (h - h0) / dt_grid;
            
            // Compute X-Y Gradients
            const float eps = 5.0f;
            const float dhdxp = (trilinear_interpolation(hgrid1, ic+eps, jc+eps, qc) - trilinear_interpolation(hgrid1, ic+eps, jc-eps, qc)) / (2.0f*eps*DS);
            const float dhdxc = (trilinear_interpolation(hgrid1, ic, jc+eps, qc) - trilinear_interpolation(hgrid1, ic, jc-eps, qc)) / (2.0f*eps*DS);
            const float dhdxm = (trilinear_interpolation(hgrid1, ic-eps, jc+eps, qc) - trilinear_interpolation(hgrid1, ic-eps, jc-eps, qc)) / (2.0f*eps*DS);
            dhdx = (dhdxp + dhdxc + dhdxm) / 3.0f;
            const float dhdyp = (trilinear_interpolation(hgrid1, ic+eps, jc+eps, qc) - trilinear_interpolation(hgrid1, ic-eps, jc+eps, qc)) / (2.0f*eps*DS);
            const float dhdyc = (trilinear_interpolation(hgrid1, ic+eps, jc, qc) - trilinear_interpolation(hgrid1, ic-eps, jc, qc)) / (2.0f*eps*DS);
            const float dhdym = (trilinear_interpolation(hgrid1, ic+eps, jc-eps, qc) - trilinear_interpolation(hgrid1, ic-eps, jc-eps, qc)) / (2.0f*eps*DS);
            dhdy = (dhdyp + dhdyc + dhdym) / 3.0f;

            // Compute Rotational Gradient
            const float q_eps = 1.0f;
            const float qp = q_wrap(qc + q_eps);
            const float qm = q_wrap(qc - q_eps);
            const float dhdyaw = (trilinear_interpolation(hgrid1, ic, jc, qp) - trilinear_interpolation(hgrid1, ic, jc, qm)) / (2.0f*q_eps*DQ);
            //dhdyaw = 0.0f;

            // Single Integrator Safety Filter
            const float issf1 = 10.0f;
            const float issf2 = 100.0f;
            const float b = dhdx*dhdx + dhdy*dhdy + dhdyaw*dhdyaw;
            const float Lgh_norm = sqrtf(b);
            float a = WN*h;
            a += dhdt; // Dynamic Environment
            a += dhdx*vxd + dhdy*vyd + dhdyaw*vyawd; // Min Norm Controller
            a -= 1.0f/issf1 * Lgh_norm + 1.0f/issf2 * b; // Robustness
        ...
        void occ_grid_callback(nav_msgs::msg::OccupancyGrid::UniquePtr msg){
            ...
            if(MPC3D_FLAG){
                ...
                for(int n = 0; n < IMAX*JMAX; n++){
                    if(q1f!=q2f) grid_now[n] = (q2f - qr) * hgrid1[q1*IMAX*JMAX+n] + (qr - q1f) * hgrid1[q2*IMAX*JMAX+n];
                    else grid_now[n] = hgrid1[q1*IMAX*JMAX+n];
                }
            }
            else{
                memcpy(grid_now, hgrid1, IMAX*JMAX*sizeof(float));
            }
            display_poisson_safety_function(grid_now);
        ...
        void state_update_callback(const unitree_go::msg::SportModeState::SharedPtr data){
            ...
            // Run MPC with Real-Time Iteration (RTI) SQP
            if(h_flag){
                const float xd[3] = {rxd, ryd, yawd};
                const float x[3] = {rx, ry, yaw};
                const float xc[3] = {rxc, ryc, yawc};
                if(MPC3D_FLAG){
                    mpc3d_controller.update_cost_and_constraints(hgrid1, xc, xd, x);
                    mpc3d_controller.solve();
            ...
            if(start_flag && !stop_flag){
                ...
                vyaw = fminf(fmaxf(vyaw, -3.0f), 3.0f);
        ...
        float vx, vy, vyaw;
        float vxb, vyb;
        float h, dhdt, dhdx, dhdy, dhdyaw;
        
        int8_t conf1[IMAX*JMAX];
        int8_t conf0[IMAX*JMAX];
        float *hgrid1, *hgrid0, *bound, *force, *robot_kernel;
