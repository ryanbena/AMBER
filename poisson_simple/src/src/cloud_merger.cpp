...
class CloudMerger : public rclcpp::Node
{
    public:
        ...
        : Node("cloud_merger"){
            ...
            for(int n=0; n<grid_length*grid_length; n++) old_conf[n] = 0;
    ...
    private:
        ...
        void lidar_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg){
            ...
            const float trimX = 0.0f;
            const float trimY = 0.0f;
            const float trimZ = 0.225f;
            const float roll_trim = -1.0f*M_PI/180.0f;
            const float pitch_trim = 0.0f*M_PI/180.0f;

            // Get curr IMU reading
            float x_pos = latest_pose_.position[0] + trimX;
            float y_pos = latest_pose_.position[1] + trimY;
            float z_pos = latest_pose_.position[2] + trimZ;
            float roll_angle = latest_pose_.imu_state.rpy[0] + roll_trim;   
            float pitch_angle = latest_pose_.imu_state.rpy[1] + pitch_trim;   
            float yaw_angle = latest_pose_.imu_state.rpy[2];
            Eigen::Quaternionf q_body(latest_pose_.imu_state.quaternion[0],
                latest_pose_.imu_state.quaternion[1],
                latest_pose_.imu_state.quaternion[2],
                latest_pose_.imu_state.quaternion[3]);
            ...
            for (const auto& pt : cloud->points){
                float ellipse_norm = std::pow(pt.x/minx,8.0) + std::pow(pt.y/miny,8.0);
            ...
            transform.linear() = R_body * R_flip;  //rotation
        ...
        void Filtered_Occupancy_Convolution(int8_t *confidence_values, const cv::Mat& occupancy_data, const int8_t *old_conf_map){
            ...
            for (int r=0; r<grid_length; r++){
                for(int c=0; c<grid_length; c++){
                    ...
                    float conf0 = (float)confidence_values[r*grid_length+c] / 127.0f;
                    float conf1;
                    if(val_binary > Th){
                        float sig1 = 1.0 - std::exp(-beta1*val_binary*dt_sec);
                        conf1 = (1.0-sig1)*conf0 + sig1*C_plus;
                        //confidence_values.at<float>(r,c) = (1.0-sig1)*conf0 + sig1*C_plus;
                    }
                    else{
                        conf1 = (1.0-sig2)*conf0 + sig2*C_minus;
                        //confidence_values.at<float>(r,c) = (1.0-sig2)*conf0 + sig2*C_minus;
                    }
                    confidence_values[r*grid_length+c] = (int8_t)(127.0f * conf1);
                }
            }
